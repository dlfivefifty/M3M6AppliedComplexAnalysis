...
\]
that is the maximum row sum and the $2$-norm \$ {\textbackslash}|A {\textbackslash}|\_2 \$ which equals the largest singular value, that is the square-root of the largest eigenvalue of $A^\top A$.
...
Then, using a slightly modified \texttt{sqrt\_n} from Lecture 6 to compute the Taylor series and  comparing with \texttt{sqrt(A)} which is Julia's implementation of matrix square root,
...
But of course if the eigenvalues become much larger this won't work. Here we see an example of a matrix whose eigenvalues are not in the  radius of convergence:
...
Using the partial sum of the Taylor series completely fails:
...
\begin{lstlisting}
(*@\HLJLn{J}@*) (*@\HLJLoB{=}@*) (*@\HLJLnf{zeros}@*)(*@\HLJLp{(}@*)(*@\HLJLni{10{\_}000}@*)(*@\HLJLp{,}@*)(*@\HLJLni{10{\_}000}@*)(*@\HLJLp{);}@*)
\end{lstlisting}


\subsection{Caucy's integral formula}
The last approach we detail in the next lecture is to use the Cauchy integral formula: \textbf{Def (Cauchy's integral  matrix function)} Suppose $\gamma$ is a simple, closed contour that surrounds the spectrum of $A$. Then define
...
\includegraphics[width=\linewidth]{figures/Lecture7_6_1.pdf}
